...
def main() -> None:
    ...
    try:
        ...
        kt_api_key = config.get('credentials', 'kt_api_key')

        login_endpoint = config.get('endpoints', 'login_endpoint')
        scores_endpoint = config.get('endpoints', 'scores_endpoint')

        # all config fields are required
        if not all([
            username, password, kt_api_key, login_endpoint, scores_endpoint
        ]):
            raise Exception('Missing one or more fields.')
    ...
    with requests.Session() as s:
        login_token = get_login_token(s, login_endpoint, username, password)
        notify('Successfully logged in to the KONAMI service.')

        scores_raw = get_raw_sdvx_scores(s, scores_endpoint, login_token)
    ...
    notify('Importing...')
    n_new_scores, n_sessions, n_errors = import_scores(kt_api_key, batch_manual)
